...
def infer_image(client, model, input_path, output_path, input_shape, data_type):
    ...
    end_time = time.time()
    print(f"{model} inference time: {(end_time - start_time) * 1000:.3f} ms")
...
def infer_camera(client, model_name, input, input_shape, data_type):
    ...
    # encoded_data = cv2.imencode(".jpg", img)[1].tobytes()
    # batch_img_byte = np.array([encoded_data], dtype=np.object_)
...
def main():
    ...
    if args.infer_mode == "img":
        output_path = f"./output/openvino/{args.folder_name}"
        mkdir(output_path)
